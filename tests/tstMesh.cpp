...
TYPED_TEST( MeshTest, LocalGridSetup) 
{
    ...
    for (int i = 0; i < 2; i++)  {
        EXPECT_EQ(this->boxCells_, global_grid.globalNumEntity(Cajita::Cell(), i));
    }
    ...
    EXPECT_EQ(ghost_local_face_space.extent(0),
	      this->boxCells_ / global_grid.dimNumBlock(1) + 2*this->haloWidth_ + 1);
    EXPECT_EQ(ghost_local_face_space.extent(1),
	      this->boxCells_ / global_grid.dimNumBlock(1) + 2*this->haloWidth_);
}
